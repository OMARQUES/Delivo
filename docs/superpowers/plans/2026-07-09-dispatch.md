...
- [ ] **Step 2: `.env.development`** — adicionar placeholders (VAZIOS = recurso desligado):
```
...
```
(`FILL_ME` aqui é intencional e documentado: valores públicos entram na configuração do Firebase pelo usuário; com FILL_ME o SW é inerte. Adicione `public/firebase-messaging-sw.js` ao `.prettierignore` se o format reclamar.)
