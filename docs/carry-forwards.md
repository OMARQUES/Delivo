...
| **Comissão por loja não é editável** — `stores.commissionBps` existe (default 0) e o ledger a lê, mas não há rota nem campo no admin para definir; comissão fica 0 até implementar (form admin de loja + rota update + validação + teste) | Plano 8 (audit) | **Bloqueia comissão real — fix curto antes de operar** |
