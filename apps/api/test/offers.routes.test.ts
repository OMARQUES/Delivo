...
beforeEach(async () => {
  ...
  const store = await createActiveStoreTestFixture({ ...base, name: 'Loja', slug: 'rota-oferta', owner: { name: 'L', email: 'loja@rota-oferta.test', password: 'senha123' } })
  const other = await createActiveStoreTestFixture({ ...base, name: 'Outra', slug: 'outra-rota-oferta', phone: '4433335555', owner: { name: 'O', email: 'outra@rota-oferta.test', password: 'senha123' } })
