...
async function submit() {
  ...
  loading.value = true
  try {
    const flow = await auth.registerDriver({
      name: name.value,
      phone: phone.value,
      email: email.value,
...
<template>
  <main class="mx-auto max-w-sm p-4">
      ...
      <form class="mt-4 space-y-3" @submit.prevent="submit">
        <input v-model="name" required placeholder="Nome completo" class="w-full rounded border p-2" autocomplete="name" />
        ...
        <input v-model="password" type="password" required minlength="15" placeholder="Senha (mín. 15)" class="w-full rounded border p-2" autocomplete="new-password" />
