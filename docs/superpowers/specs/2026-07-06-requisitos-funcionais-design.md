...
**Data:** 2026-07-06
**Status:** aprovado em brainstorming (12 decisões + 10 defaults validados com o dono do produto)
...
- Loja tem: nome, slug único, endereço + pin, telefone/WhatsApp, horário de funcionamento por dia da semana, botão **"pausar pedidos agora"**, configuração de entrega (ver §6), modo de entrega (entregador próprio e/ou freelance).
- Produtos organizados em **categorias** (da loja). Produto: nome, descrição, preço, foto (R2), disponível/indisponível.
...
- **Estoque MVP = toggle** disponível/indisponível por produto e por variação. Contagem numérica: plano ERP, pós-MVP.
...
- Pagamento **online**: pedido nasce `AWAITING_PAYMENT`; só aparece pra loja após confirmação (webhook Asaas). PIX expira em **15 min** → auto-cancel.
...
- **Frete por loja, dois modos** (loja escolhe):
  - `FIXED`: valor único.
  - `DISTANCE`: valor mínimo + R$/km, distância **haversine** (linha reta) do pin da loja ao pin do cliente, arredondada pra cima em passos de 0,5 km. Sem API de rotas.
...
- Gestão de catálogo (categorias, produtos, variações, adicionais, fotos, disponibilidade).
- Horário + pausar pedidos.
...
- Bloqueio/desbloqueio de loja e entregador.
- Visão geral: pedidos do dia, GMV, comissões.

## 12. Fora do MVP (explícito)

Agendamento de pedido · avaliações/notas · cupons/promoções · chat in-app · otimização de rota · GPS em tempo real · contagem de estoque · self-service de loja · OTP SMS/WhatsApp · notificação WhatsApp automática · código de retirada · app iOS.

## 13. Impactos técnicos já mapeados

- `packages/shared`: máquina de estados ganha `AWAITING_PAYMENT` e transições `READY→OUT_FOR_DELIVERY` / `READY→DELIVERED` (pickup); labels novos. Plano de fundação (Task 2) deve incorporar.
...
- Roadmap de planos revisado: 2. Auth · 3. Catálogo · 4. Pedidos (estados, amendment, cancelamento, pickup) · 5. Dispatch (broadcast, lock, batching) · 6. Pagamentos Asaas (split, estorno) · 7. Financeiro (ledger, fatura, payout) · 8. Capacitor/FCM · 9. Admin & Relatórios.
