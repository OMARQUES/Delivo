...
describe('payment reconciliation', () => {
  it('processes due inbox with bounded limit and reports counts only', async () => {
    ...
    const summary = await runPaymentReconciliation(testDb, provider(), now, { resolvePayerEmail: (email) => email ?? 'test@test.local' }, { inbox: 1, operations: 0, creates: 0, snapshots: 0, expirations: 0, reviews: 0 })
  ...
  it('failure in one stage does not prevent summary return', async () => {
    ...
    const summary = await runPaymentReconciliation(testDb, broken, new Date(), { resolvePayerEmail: (email) => email ?? 'test@test.local' }, { inbox: 1 })
